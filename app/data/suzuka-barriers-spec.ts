 ...
 * at the verges (0.75 m); guardrail3 — the white triple-beam rail of the 130R verges (0.95 m,
 * r130.jpg; I4-c); concrete — white concrete wall (1.05 m); tyre — belt-covered tyre wall
 * (1.5 m since I4-c: the photos read 1.2–1.5); fence — chain-link only (no wall), e.g. the
 * car-park fence behind the pit entry lane.
 */
export type BarrierKind = 'armco' | 'guardrail' | 'guardrail3' | 'concrete' | 'tyre' | 'fence'

/** the debris fence's colour: the historic dark green, or the 2024 black repaint (web-suzuka: C stand, T3, Spoon …) */
export const FENCE_GREEN = '#1f5a34'
export const FENCE_BLACK = '#101214'
...
export interface BarrierRun {
  ...
  fence?: number
  /** the fence covers only this part of the run (s, inside sRange); default the whole run */
  fenceRange?: [number, number]
  /**
   * 'both' = a second fence on the spectator side of the wall as well (the double fence of
   * 逆バンク and the R / S stands, web-suzuka), FENCE_BACK_SETBACK behind the wall's back; default
   * 'track' = the one fence on the wall
   */
  fenceSide?: 'track' | 'both'
  /** tint of that fence's mesh and posts (sRGB hex); default FENCE_GREEN, FENCE_BLACK = the 2024 repaint */
  fenceColour?: string
  /** a φ0.06 top rail and two mid cables on the fence (every fenced run since I4-c) */
  topRail?: boolean
  /** photographers' windows: 1.2 × 0.8 m openings cut in the fence mesh, centred at these s (a photographer stands behind each — ops-spec `windowSlots`) */
  windows?: number[]
  /** marshals' gates: [s0, s1] cuts (≈ 1.2 m) in the fence with a framed gate leaf */
  gates?: [number, number][]
  /**
   * the wall's face: 'belt' (tyre: white conveyor belting, default) / 'concrete' (default for
   * walls) / 'painted-rwg' — the red / white / green blocks (tyre walls: `paintedBlockTexture`,
   * 2 m blocks) or the tricolour bands of the T2 wall (concrete: `tricolourWallTexture`), over the
   * `painted` s ranges
   */
  face?: 'belt' | 'painted-rwg' | 'concrete'
  /** the s ranges of the run whose face is painted (`face: 'painted-rwg'`) */
  painted?: [number, number][]
  /** advertising boards on the track face (concrete runs only, A11): the grandstand front wall */
  boards?: boolean
  /** a hoarding band of this height (m) standing on the wall's top (BOARD_TEXTS, the wall-top ad bands of I4-c) */
  adBand?: number
  unverified?: string[]
  note?: string
}
...
export const BARRIERS: BarrierRun[] = [
  ...
  { id: 'gs-front', kind: 'concrete', side: 1, sRange: [5546, 362], fence: 2.6, topRail: true, boards: true, adBand: 0.9, windows: [40, 5700, 5640], source: { osm: [471430732, 471430724, 471430725, 471430720, 471430723, 471430721, 469591632] }, note: 'grandstand / VIP / A1 front wall, boards on the track face, debris fence on top' },
  { id: 'a-stand-front', kind: 'concrete', side: 1, sRange: [362, 446], fence: 2.6, topRail: true, gates: [[426, 427.2]], source: { osm: [470125557], samples: P([[380, 15.5], [420, 17.0], [440, 17.5]]) } },
  { id: 't1-outside', kind: 'concrete', side: 1, sRange: [446, 470], fence: 2.6, topRail: true, source: { osm: [469261663] } },
  { id: 't1-t2-outside-tyres', kind: 'tyre', side: 1, sRange: [470, 640], fence: 2.6, topRail: true, source: { osm: [469261663, 469261665, 469591634] }, note: 'white belt-faced tyre wall behind the T1/T2 asphalt run-off, B stands behind' },
  { id: 't2-exit-outside', kind: 'concrete', side: 1, sRange: [640, 684], fence: 2.6, topRail: true, adBand: 0.9, source: { osm: [469591634] } },
  { id: 'c-foot', kind: 'guardrail', side: 1, sRange: [684, 960], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, source: { samples: P([[684, 25.0], [700, 17.0], [730, 15.0], [760, 14.0], [790, 14.0], [820, 14.0], [850, 14.0], [880, 14.0], [910, 14.0], [940, 15.0], [960, 15.5]]) }, unverified: ['aerial only (no OSM way at the foot of the C terrace); ±4 m'] },
  { id: 'esses-outside', kind: 'concrete', side: 1, sRange: [960, 1240], fence: 2.6, topRail: true, gates: [[1016, 1017.2]], source: { samples: P([[960, 15.5], [980, 18.0], [1010, 22.0], [1040, 26.0], [1070, 28.0], [1100, 29.0], [1130, 29.0], [1160, 27.0], [1190, 24.0], [1220, 22.0], [1240, 17.5]]) }, unverified: ['aerial only; gravel outer edge in front of D5 / D1–4, ±5 m'] },
  { id: 'gyaku-outside', kind: 'concrete', side: 1, sRange: [1240, 1377], fence: 2.6, fenceSide: 'both', topRail: true, source: { samples: P([[1240, 17.0], [1250, 17.0], [1260, 17.5], [1270, 19.5], [1285, 22.5], [1292, 25.0], [1312, 25.5], [1322, 26.0], [1330, 30.0], [1340, 29.5], [1360, 27.5], [1370, 26.5], [1377, 30.0]]) }, unverified: ['aerial + OSM sand outer edge; ±2 m'] },
  { id: 'gyaku-exit-outside', kind: 'concrete', side: 1, sRange: [1377, 1424], fence: 2.6, topRail: true, source: { osm: [468009620] } },
  { id: 'e2-toe', kind: 'concrete', side: 1, sRange: [1424, 1526], fence: 2.6, topRail: true, source: { osm: [467879578] } },
  { id: 'nippo-exit-inside', kind: 'guardrail', side: 1, sRange: [1526, 1682], fence: 2.6, topRail: true, source: { samples: P([[1526, 18.6], [1530, 19.2], [1540, 18.2], [1550, 15.7], [1560, 13.7], [1570, 9.7], [1580, 8.1], [1600, 8.1], [1620, 7.6], [1640, 8.0], [1660, 10.0], [1682, 12.0]]) }, unverified: ['aerial only; ±2 m'] },
  ...
  { id: 'degner2-trap-back', kind: 'tyre', side: 1, sRange: [2100, 2203], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, source: { osm: [467454306, 689143101] }, unverified: ['fence (I4-c): the 2026 photos show a black mesh behind the Degner 2 trap; height estimated'] },
  ...
  { id: 'chicane-escape-wall', kind: 'concrete', side: 1, sRange: [5160, 5172], fence: 2.6, topRail: true, source: { osm: [470173101], project: 'ray', reach: 60 }, note: 'west leg of the Q2 bank wall, from the end of the P tyre wall to the mouth of the escape road; 12 m of s sweep 32 m of wall' },
  { id: 'chicane-q2-front-wall', kind: 'concrete', side: 1, sRange: [5235, 5335], fence: 2.6, topRail: true, windows: [5300], source: { osm: [470173101], project: 'ray', reach: 85 }, note: 'the rest of the same wall, round the back of the chicane triangle and in front of the Q2 bars and Q1; +62 m at the T17 apex, closing to +17 at the T18 entry' },
  { id: 't18-outside-tyres', kind: 'tyre', side: 1, sRange: [5335, 5452], fence: 2.6, fenceSide: 'both', topRail: true, source: { osm: [468778567] } },
  { id: 's-front', kind: 'tyre', side: 1, sRange: [5452, 5546], fence: 2.6, fenceSide: 'both', topRail: true, source: { osm: [471430726, 469931177] } },
  ...
  { id: 't1-inside-island', kind: 'concrete', side: -1, sRange: [330, 400], adBand: 0.9, source: { osm: [469065003], samples: P([[400, -28.0]]) } },
  { id: 't1-t2-inside', kind: 'concrete', side: -1, sRange: [400, 870], adBand: 0.9, face: 'painted-rwg', painted: [[560, 690]], source: { osm: [469048580, 469064999], samples: P([[400, -28.0], [410, -22.0], [430, -17.0], [450, -14.5], [600, -17.5], [640, -17.0], [680, -15.0]]) }, unverified: ['584–706 interpolated: the two OSM ways leave a gap at the T2 exit'] },
  { id: 't3-outside', kind: 'concrete', side: -1, sRange: [870, 1040], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, source: { samples: P([[870, -13.0], [890, -18.0], [920, -22.0], [950, -22.0], [980, -19.0], [1000, -15.0], [1020, -14.0], [1040, -15.5]]) }, unverified: ['aerial only (OSM 469065004 diverges into the basin); ±4 m'] },
  ...
  { id: 'gyaku-inside', kind: 'concrete', side: -1, sRange: [1240, 1313], fence: 2.6, topRail: true, source: { osm: [468313203] } },
  ...
  { id: 'dunlop-outside-tyres', kind: 'tyre', side: -1, sRange: [1774, 1884], fence: 2.6, topRail: true, source: { osm: [467219910] } },
  { id: 'dunlop-exit-tyres', kind: 'tyre', side: -1, sRange: [1884, 1994], fence: 2.6, topRail: true, source: { osm: [467386928] } },
  { id: 'crossover-abutment-right', kind: 'concrete', side: -1, sRange: [2296, 2346], source: { samples: P([[2296, -8.5], [2321, -8.5], [2346, -8.5]]) } },
  { id: 'g-cross-front', kind: 'guardrail', side: -1, sRange: [2346, 2480], fence: 2.6, topRail: true, source: { samples: P([[2346, -8.5], [2370, -9.0], [2400, -9.0], [2430, -9.0], [2460, -9.0], [2480, -10.0]]) }, unverified: ['aerial only'] },
  { id: 'hairpin-approach-right', kind: 'concrete', side: -1, sRange: [2495, 2628], fence: 2.6, topRail: true, gates: [[2521, 2522.2]], source: { osm: [184004014], samples: P([[2495, -10.3]]) }, note: 'H stand behind' },
  { id: 'hairpin-outside-tyres', kind: 'tyre', side: -1, sRange: [2628, 2750], fence: 2.6, topRail: true, adBand: 0.9, windows: [2700], face: 'painted-rwg', painted: [[2704, 2720]], source: { osm: [183999765], samples: P([[2705, -31.0], [2715, -26.0], [2725, -21.0], [2735, -17.0], [2745, -14.5], [2750, -13.6]]) }, note: 'I stand behind; 2700–2750 read off the aerial (the OSM way stops at the apex)' },
  { id: 'hairpin-exit-right', kind: 'concrete', side: -1, sRange: [2750, 2925], fence: 2.6, topRail: true, adBand: 0.9, source: { osm: [183999770, 183999781] } },
  ...
  { id: 'l-yard-edge', kind: 'guardrail', side: -1, sRange: [3265, 3509], fence: 2.6, topRail: true, source: { samples: P([[3265, -11.0], [3280, -11.5], [3290, -12.0], [3300, -15.0], [3310, -16.5], [3320, -18.6], [3330, -20.8], [3340, -21.8], [3350, -24.5], [3360, -26.0], [3370, -26.9], [3380, -31.4], [3390, -32.4], [3400, -33.2], [3410, -33.7], [3420, -33.7], [3440, -31.8], [3460, -31.7], [3480, -30.7], [3500, -33.0], [3509, -31.1]]) }, unverified: ['aerial only (yard-edge wall or rail); ±3 m'] },
  { id: 'l-front', kind: 'concrete', side: -1, sRange: [3509, 3597], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, source: { osm: [184104882] } },
  { id: 'spoon1-outside-tyres', kind: 'tyre', side: -1, sRange: [3597, 3677], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, gates: [[3610, 3611.2]], source: { osm: [184104883] } },
  { id: 'spoon2-outside', kind: 'concrete', side: -1, sRange: [3677, 3794], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, windows: [3760], source: { osm: [184104881], samples: P([[3730, -46.5], [3740, -46.2], [3750, -43.4], [3760, -40.5], [3770, -37.2], [3780, -30.5], [3790, -29.5]]) }, unverified: ['3721–3794 aerial only'] },
  { id: 'spoon-exit-outside', kind: 'concrete', side: -1, sRange: [3794, 3891], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, source: { osm: [183953795] }, note: 'N stand behind' },
  ...
  { id: 'west-straight-trap-wall', kind: 'concrete', side: -1, sRange: [4324, 4521], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, source: { samples: P([[4324, -9.0], [4335, -14.6], [4353, -22.4], [4387, -19.4], [4440, -20.5], [4503, -22.5], [4515, -16.3], [4521, -9.2]]) }, note: 'O stand (2026) behind', unverified: ['aerial gravel outer edge'] },
  ...
  { id: 'bridge-parapet-right', kind: 'concrete', side: -1, sRange: [4585, 4740], fence: 2.6, topRail: true, source: { samples: P([[4585, -7.5], [4665, -7.5], [4740, -7.5]]) }, note: 'dark-green mesh like the left parapet (photo)' },
  { id: '130r-outside-verge', kind: 'guardrail3', side: -1, sRange: [4740, 4772], source: { samples: P([[4740, -8.0], [4772, -29.0]]) }, unverified: ['transition from the deck parapet to the 130R tyre walls'] },
  { id: '130r-outside-tyres', kind: 'tyre', side: -1, sRange: [4772, 4990], fence: 2.6, fenceColour: FENCE_BLACK, topRail: true, face: 'painted-rwg', painted: [[4953, 4969]], source: { osm: [467386927, 467386917] }, note: 'post 26 behind it (r130_post.jpg): the black fence, the red / white / green blocks 8 m either side of the post', unverified: ['fence height'] },
  ...
  { id: '130r-exit-wall', kind: 'concrete', side: -1, sRange: [4997, 5085], adBand: 0.9, source: { osm: [184102364] }, unverified: ['starts 10 m outside the end of the 130R tyre wall — check the joint on the overlay'] },
  ...
  { id: 't18-pit-entry-separator', kind: 'concrete', side: -1, sRange: [5389, 5538], fence: 2.2, topRail: true, source: { osm: [471532694], samples: P([[5520, -10.5], [5538, -9.7]]) }, note: 'between the track and the pit-entry lane; the pit wall (pit-lane.ts) continues from 5538' },
]
 ...
 * the round red-bordered 60 at the pit entry; pitEntry — the white 'PIT ENTRY' board with a green
 * arrow on the top of the pit-entry separator wall (I4-c). There are no corner-number boards at Suzuka.
 */
export type SignKind = 'drs' | 'fireStation' | 'pitExit' | 'pitExitLight' | 'speed80' | 'speed60' | 'pitEntry'
...
export interface SignDef {
  ...
  mount?: SignMount
  /** mount 'barrierTop': the BARRIERS run whose resolved line the board stands on (`lateral` is nominal then) */
  run?: string
  unverified?: string[]
  note?: string
}
...
export const SIGNS: SignDef[] = [
  ...
  { id: 'drs-detection', kind: 'drs', s: 5150, lateral: -27, facing: '-s', height: 2.0, width: 1.2, boardHeight: 1.2, note: 'DRS detection line (CIRCUIT.drs.detection); behind chicane-approach-right-b, outside the right apron (its edge is −24 at s 5148)', unverified: ['lateral: the board is not resolvable in the aerial; placed on the first unpaved ground beside the line'] },
  { id: 'drs-zone', kind: 'drs', s: 5590, lateral: 'cameraSide', facing: '-s', height: 2.0, width: 1.2, boardHeight: 1.2, note: 'DRS activation (CIRCUIT.drs.start); 2.0 m up since I4-c' },
  // the pit entry: 'PIT ENTRY' with a green arrow on the separator wall's top, 0.5 m over the wall (structures.ts draws it on the run's resolved line)
  { id: 'pit-entry', kind: 'pitEntry', s: 5400, lateral: -7.8, facing: '-s', mount: 'barrierTop', run: 't18-pit-entry-separator', height: 0, width: 1.2, boardHeight: 0.6, note: 'on the pit-entry separator wall at the lane\'s mouth; the board stands on the run\'s resolved line (−9.9 there), the lateral is nominal', unverified: ['s ±20', 'the board\'s design'] },
...
// ---------------------------------------------------------------- advertising panels (I4-c)

/**
 * The free-standing hoarding panels behind the barriers (BOARD_TEXTS words, `bigPanelTexture`):
...
export interface AdPanelDef {
  ...
  lateral: number
  facing: '-s' | '+s' | '+lat' | '-lat'
  /** board size (m); the bottom edge is `height` above the ground */
  width: number
  boardHeight: number
  height: number
  unverified?: string[]
