...
node scripts/audit/infield-smoke.mjs --tier both --cuts   # the I-phase smokes (dev-only, not in pnpm check): pit- / paddock- / ops- / trackside- / infield-smoke, `--tier high|low|both`, `--glb` for the pack drops
node scripts/facilities/build-facilities.mjs --add-ways-from .cache/overpass/surroundings.json --role "apron:467386920;parking:469079400"   # splice OSM ways in from a cached response (no network); the ids are kept across a full regeneration
node scripts/facilities/dem-profile.mjs --cuts             # the CUT corridors' long sections (and --relief for the relief zones' edges, --verify for the 34 stations)
node scripts/shots.mjs --tier 1 --assets 1 --preset garage-front   # shot-presets.mjs holds the viewpoints (shots + perf-probe --views share it)
```
...
### Runtime flow
- `app/components/RaceViewport.client.vue` is the single orchestrator: `createScene` → `loadAssets` → `environmentSteps` → `buildTrackMeshes` / `buildBarriers` / `buildLines` → `freezeStatic` → cars → `env.farField.start()` (deferred far-field jobs after the loading screen). Every step is announced to the loading screen first (`LoadTracker.stage` / `.drain`, `app/three/loading.ts`), which repaints between stages: the environment is a `Steps<T>` generator (`app/three/steps.ts`; each `yield` names the stage that starts = a `buildMs` key, `plan/…` / `meshes/…` inside the ground plan and meshes), and Node / the audits call the synchronous `buildEnvironment` = `runSteps(environmentSteps(…))`. A new builder stage wants a `STAGES` row (label, detail, measured ms per tier from `window.__suzuka.load`); time anything that spans a `yield` with `buildClock()`, which leaves out the paints. Its `loop()` steps `RaceSim`, places cars, effects, cameras, audio, then renders and syncs the HUD store. Inside `buildEnvironment`, `infield.ts buildInfield` (pit lane → paddock → ops → marshal posts + TV towers → infield ground → infield water → cuttings) runs synchronously right after `buildPitComplex`; its props go through `props-pack.ts` (GLB-or-procedural prototypes, `Quality.infield`), `infield-lod.ts registerPropSet` (per-cell InstancedMesh LOD on the far-field registry, kinds `ops` / `infield`) and `figures.ts` (people, shared with the crowd).
...
- There are no numeric baselines: every non-zero tolerance is an `ALLOWANCES` entry in `scripts/audit/surface-check.mjs` with a `why` and an `until` phase, and it fails once `PHASE` reaches it. `PHASE` is **'P6i'** (the I phase; `PHASES` = P0 … P6, P6s, P6i, P7), so a new allowance is `until: 'P7'` with a `why` from the `WHY` table — never a bare number, and never a loosened guard to make a build pass. `--suggest` prints the entry for a failing key; you still write the reason.
- Mesh names carry the G8 contract: the prefixes `stand / terrace / furniture / props / concrete / pitRails / pitInterior / terrain` are exempt from the registration guard, so name a mesh `props-*` or `furniture-*` **only** when it has no horizontal face ≥ 5 m² at ground height (±0.3 m of the ground, inside 75 m of the centreline). Anything with such a face is a `GroundFace` (a table row), a `markDecal` decal or a `markObject` `GROUND_OBJECTS` row — `props-` in its name does not make it legal.
...
### Data files
Hand-written specs: `suzuka.ts`, `suzuka-facilities-spec.ts`, `suzuka-barriers-spec.ts`, `surroundings-spec.ts` (road sections `roadSectionOf`, `ROADS`, `ROAD_FURNITURE`, `FOREST`, `HERO_BUILDINGS`, `PADDY` / `RAIL` / `STREAM`), `tree-species.ts` (roles → pack node regexes / LODs / heights / tints / `TREE_MIX`), `drivers.ts`, `ops-spec.ts` (the static ops layer inside the fences: `OpsPlacement` rows, `opsPlacements()` / `figuresAt()` / `OPS_TEXTS`, pure data + pure functions, no three; read by `ops.ts` and by `facilities-check` §16 ops-check), `infield-trees.ts` (`infieldTreePlacements(track)`, the pure expansion of `INFIELD_TREES` that `vegetation.ts` and `facilities-check` O10 both read). The I phase's tables live in the two specs: `suzuka-facilities-spec.ts` carries `GARAGE_CENTRES` / `PIT_BUILDING` / `PIT_WALL` / `PIT_ENVELOPE` / `PIT_PLANNED`, `PADDOCK_BUILDINGS` / `PADDOCK_OFFICE` / `PADDOCK_FENCE` / `PADDOCK_LAMPS` / `PADDOCK_MASTS` / `PADDOCK_PARKING` / `PADDOCK_BAY`, `INFIELD_FACILITIES` / `INFIELD_PARKING` / `INFIELD_LAMPS` / `INFIELD_ISLAND_KERBS` / `INFIELD_TREES`, `SOUTH_COURSE_KERBS`, `CUTS` / `STAIR_PIT` / `CUT_WALL_FOOT` and `FOOTBRIDGES` / `FOOTBRIDGE`; `suzuka-barriers-spec.ts` carries `MARSHAL_POSTS` v2 (+ `marshalNumbers()`), `TV_CAMERAS`, `TRACKSIDE_CCTV`, `TYRE_STACK` and `AD_PANELS`. **Generated — never hand-edit** (the header says so): `suzuka-facilities.ts`, `suzuka-surroundings.ts`, `suzuka-power.ts` (OSM, ODbL; `scripts/facilities/build-*.mjs`), `suzuka-dem.ts` (GSI DEM; `dem-profile.mjs --grid --far --write`), `credits.ts` and `CREDITS.md` (`scripts/assets/import-misc.mjs`). The surroundings extract ships road `lanes / surface / maxspeed / width`, stream `tunnel`, rail `electrified`, land `leaf_type / leaf_cycle / wood`, a `greenhouse` building kind, and keeps every node shared by two highway ways through the 1 m DP (the runtime's junctions are found by vertex coincidence). Raw downloads and user reference material live in `misc/` (gitignored); `public/assets/` is the converted, committed output.
...
### Budgets
`scripts/perf-budgets.json` holds the static budgets (`scene-cost`, strict in `pnpm check`) and the browser ceilings (`perf-gate`, formulas in its `comment`). Re-base only from a measured run, and say which one. The static budgets are re-based **on the commit that exceeds them**, to that commit's measured `scene-cost --tier high|low` value × 1.10 (triangles rounded up to 10k, counts to 10), with `measuredAt` naming the phase, the measured numbers and what added them; the browser ceilings are re-based once per phase from a `LABEL=<name> pnpm perf` run on a :3100 dev server, never by scaling old numbers.
...
## Policies
- Assets: CC0 and CC-BY (with credit) only, ≤ 200 MB committed in `public/assets/` (≤ 512 MB estimated VRAM); no real logos, sponsor names or trademarks in generated textures (`textures-lint.mjs` with `scripts/trademark-*.json`) — vehicle and ops GLBs get their badge / plate rectangles blurred at import (`retouch` in `sources.mjs`, or `retouchReviewed` saying why none is needed). Sketchfab CC-BY drops live under `misc/<group>/` (`MISC_GROUPS`: trees / road / buildings / vehicles / seats / ops / trackside / pit) and are optional: every builder keeps a procedural fallback.
