...
const WHY = {
  ...
  cutWalls:
    'P6i (I6, the cuttings and tunnels): a CUT corridor lowers the field between its walls (README R6, I6 改訂), and the 0.6 m foot where the wall meets the floor is a band steeper than 10° but flatter than the 45° cliff threshold — it is neither a walkable face nor a cliff to the guards, so it counts as steep on the kinds it crosses; G4 also reads the field normal at the rounded metre (±0.5 m), so a floor or verge triangle within a metre of a wall is judged against the wall\'s normal and counts as steep although it lies flat on its own field — the corridors\' gravel rings (the 0.5–2 m band between the road ring and the wall) are all such triangles. I6-a measured on both tiers (--suggest): G3 asphaltArea 4.07 → 4.19 %, gravelArea 7.50 → 8.31 %; G4 gravelArea.steep 216 → 3,712, asphaltArea.steep 1,994 → 2,979, grass.steep 1,890 → 2,662, lane.steep 54 → 69, gravelBand.steep 56 → 60, asphaltBand.steep 338 → 352. The I6 review re-based them: the corridors now carry a third ring AT the wall foot, so the drawn shoulder is the 0.6 m foot itself instead of one chord from the rim to the road edge (it was 60–70° and stood in front of the concrete face), and the field is the floor over the whole corridor (the sampler missed interior points on a curving way). Measured again on both tiers: G3 gravelArea 8.31 → 5.61 %, asphaltArea 4.19 → 4.05 %; G4 gravelArea.steep 3,712 → 3,045, asphaltArea.steep 2,979 → 2,722, gravelBand.steep 60 → 53, grass.steep 2,662 → 2,699, asphaltBand.steep 352 → 362, lane.steep 69 → 77 (the last three rose: the floor the sampler had missed is now cut, so the faces beside those walls are judged against a wall normal). A vertical wall face (P7: a true cliff row in the field, or the foot drawn as the wall\'s own geometry, I6-b\'s retaining walls covering the foot) removes it',
  osmGap:
...
const ALLOWANCES = [
  ...
  { guard: 'G3', key: "ground:gravelArea", bound: 5.9, why: `${WHY.fieldCells} ${WHY.cutWalls}`, until: 'P7' }, // the I6 review (R2 / V1): measured 5.61 on both tiers (--suggest), was 8.31 at I6-a (7.5 before it) — the corridors' gravel now has its own ring AT the wall foot and only the 0.6 m foot chords
  { guard: 'G3', key: "ground:gravelBand", bound: 1.77, why: `${WHY.fieldCells} ${WHY.infieldRings}`, until: 'P7' }, // I5-c: measured 1.68 high (--suggest), was 1.55 — the dry basins' shore rings (grown OSM discs) re-station the S-curve leg's gravel band beside them (the worst, s 1080–1108, is the one that was there)
  { guard: 'G3', key: "ground:asphaltArea", bound: 4.26, why: `${WHY.fieldCells} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // the I6 review: measured 4.05 on both tiers (--suggest), was 4.19 at I6-a (I5-a 3.87, before that 0) — the corridors' road rings at the daylight ends
  ...
  { guard: 'G3', key: "ground:water", bound: 14.65, why: `${WHY.fieldCells} ${WHY.demCurvature} ${WHY.infieldRings}`, until: 'P7' }, // I5 review: measured 13.95 low (--suggest; high 12.0), was 12.49 (I5-c 11.76–11.89 high, I5-a 10.86, before that 9.3) — the ponds' planes were set against the lowest road plane in their windows (the west pond 1.5 m deeper at its NE end over a 16 m bank, the 130R pond's bank 10 m + the island's own 4 m), so more of the two banks curve; the same chording of the field by the 2 m raster rows
  { guard: 'G4', key: "ground:asphaltArea.steep", bound: 2859, why: `${WHY.fieldCells} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // the I6 review: measured 2722 on both tiers (--suggest), was 2979 at I6-a (1994 before it, I5-a 1899) — the corridors' road rings within a metre of the walls
  { guard: 'G4', key: "ground:asphaltBand.steep", bound: 370, why: `${WHY.fieldCells} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // I6-a: measured 352 on both tiers (--suggest), was 338 (I5-a 321, before that 280) — the band beside the pedestrian pits' walls
  { guard: 'G4', key: "ground:grass.steep", bound: 2796, why: `${WHY.fieldCells} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // I6-a: measured 2662 on both tiers (--suggest), was 1890 (I5-a 1800, I2-a 1553) — the verge grass within a metre of the corridor walls
  { guard: 'G4', key: "ground:grassArea.steep", bound: 35, why: WHY.fieldCells, until: 'P7' },
  { guard: 'G4', key: "ground:gravelArea.steep", bound: 3198, why: `${WHY.fieldCells} ${WHY.demCurvature} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // the I6 review: measured 3045 on both tiers (--suggest), was 3712 at I6-a (216 before it, I5-c 205) — the corridors' gravel is the 0.6 m foot plus the shoulder ring beside it
  { guard: 'G4', key: "ground:gravelBand.steep", bound: 56, why: `${WHY.fieldCells} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // the I6 review: measured 53 on both tiers (--suggest), was 60 at I6-a (56 before it, I5-a 53) — a run-off band within a metre of a corridor wall
  ...
  { guard: 'G4', key: "ground:lane.steep", bound: 81, why: `${WHY.fieldCells} ${WHY.infieldRings} ${WHY.cutWalls}`, until: 'P7' }, // the I6 review: measured 77 on both tiers (--suggest), was 69 at I6-a (54 before it, I5-a 51) — a lane within a metre of a corridor wall: the field is now the floor over the WHOLE corridor (R1), so the pit lane beside worksNE / worksSW reads eight more triangles against a wall normal
  ...
  { guard: 'G4', key: "ground:water.steep", bound: 1581, why: `${WHY.fieldCells} ${WHY.demCurvature} ${WHY.infieldRings}`, until: 'P7' }, // I5 review: measured 1505 on both tiers (--suggest), was 1417 (I5-c 1349, before that 1040) — the ponds' planes set against the lowest road plane in their windows: the west pond's NE end falls 6–7 m over its 16 m bank, the 130R pond's island has its own 4 m bank (the outer bank's width would leave no floor between the two shores)
