...
- **面の舗装**（`SURFACE_PATCHES` in `app/data/suzuka-facilities-spec.ts` → `app/three/surfaces.ts`）：
  ランオフは基本的に「中心線からの横方向の帯」を s に沿って掃引して作りますが、それでは表せない場所が
  あります。アステモシケインのカシオトライアングルは、GP コース・エスケープロード・二輪ダブルシケイン・
  ランオフが 1 枚の連続したアスファルトで、左は +45 m まで届きます。一方この区間の曲率半径は 20〜23 m
  しかないため、内側に約 16 m 以上出した帯は**掃引フレームが反転して自己交差します**
  （`app/three/ground.ts` の `FOLD_SAFE`。全周で 6 か所該当し、以前はそこが実際にちぎれて描かれていました）。
