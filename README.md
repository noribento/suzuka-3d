...
app/
  ...
  three/
    ...
    ground-field.ts            # 唯一の連続した高さ場（路肩 2 m のストリップ規則、2〜8 m の混合、地形 + RUNOFF_LIFT）と CutField（I6: CUTS の廊下 = ポータルから heading へ grade で上がる床、壁裾 0.6 m の smoothstep、路面フレーム・BARRIERS 線・フィル列を避けた始端、`cutAt` / `yNoCut`、地面行の多角形 `corridor(id, part)`（corridor / foot / road）と置く側の `keepOut(id, margin)`。「切通しとトンネル（R6）」参照）
...
- **観客**：座席位置はスタンド生成器が返し、観客はその席に座ります（決勝の占有率 95 %、西エリアは疎）。高品質ティアは
  ...
  **占有の抽選が先**、そのあと 60 m ベイごとの誤差拡散で `rate = min(1, quality.crowd / 埋まった席)` を掛けるので、
  ...
  約 82.7 k で、実物の公称値まで下げるには残りのスタンドの公式席数が要ります（`env.stats.seats`。I6 レビューで A1 仮設の
  s 範囲を構内道路トンネル北東進入路の手前で切ったので 83.7 k から 1 k 減りました）。
- **芝土手の観客**（`app/three/banks.ts`）：逆バンクオアシス・E の丘・ヘアピン外側・J・西エリアの L / M / N・S 席の先の
  ...
  `Ground.standY` の上（`SeatSlot.kind: 'lawn'`）— ただし CUT の廊下から 1 m 以内は捨てます（`BankStats.inCut`、
  I6 レビュー R6 / V5: 逆バンクランプと歩行者ピットが S 席の先の芝土手を貫き、観客が塹壕の床に立っていた）— なので、観客のベイ・LOD・アトラス・予算がそのまま効きます。座った塊の
...
- **地面の契約**（`app/three/ground.ts`、`ground-plan.ts`、`ground-mesh.ts`、`ground-field.ts`。`scripts/audit/surface-check.mjs` が
  ...
  - **R6 フレーム**。`road / kerb / deckShoulder / pitLane / pitApron` は路面平面（縁石は自分の横位置での断面）、他は高さ場。
    ...
    **I6 改訂 — 場は宣言された CUT に従う**: `CUTS`（`suzuka-facilities-spec.ts`）の各行の**廊下多角形**の内側では `field.y` は
    ...
    外側は従来の規則そのもの — `yProjected` は廊下の標本が無い点で `yNoCut` と同じ文を走るので**構成上**同一です（I6 の
    `dem-profile --cuts` の「同一性」は同じ述語どうしの比較で、決して落ちない検算でした。I6 レビュー R8 で、落ちうる 2 つ:
    廊下の内側は床そのもの（埋めない・床を割らない・壁裾 0.6 + 0.25 m より内はビット同一の床、1.35 万点）と、標本器の
    **閉じ込め**（多角形 + 壁裾より外の 2 m 格子 219 万点で `cutAt` null かつ `y === yNoCut`）に置き換えました）。
    廊下は authoring しない — ポータル・向き・
    ...
    （`{ cut: id, part }` 足跡: 全幅の gravelArea layer 0 / 壁裾までの床 gravelArea layer 1（`foot`）/ 路面 ± 3.5 の
    asphaltArea layer 2、階段ピットは asphalt の 2 重リング。**壁裾のリングはラスターに壁裾のカラムを立てるため**で、
    無いと 0.6 m の裾が路肩の全幅（0.9〜1.4 m）に弦を張り、擁壁の面の手前に 3〜4 m の土手を描きました（I6 レビュー R2 / V1。
    砂利の路肩が 0.3 m 未満の 4.0 / 3.5 m ランプと階段ピットは `cutHasFootRing` が false で、路面リングが裾そのものに届く））、
...
- **データ** `CUTS`（`suzuka-facilities-spec.ts`、全行 unverified）: `CutDef { id, osmWay?, portal, window, depth, grade, halfWidth,
  ...
  `worksNE`（175231859 first）/ `worksSW`（手置き (117, −28, 231)、エプロン −28.7 の外。**4.0 m / 16 %**（1:6 の作業車ランプ）で
  d ≈ 24 = lateral −52 で地上へ: 計画の 5.5 m / 8 % は 66 m 掘れてパドック道路 (116, −53) を横断し給油所の前庭まで達していた
  — I6 レビュー R4 / V3）、`r200service`（(3190, +13, 200):
...
- **場** `buildCutField(track, terrain, cuts)`（プランより前に組む: プランの `{ cut }` 行が廊下を要るので、路面フレームは
  ...
  32 m セルの索引 → 廊下の箱 → 中心線サンプルへの射影（隣サンプルとの幅の補間）。重なる廊下は深い方。`corridor(id, part)` は
  `corridor` = 全幅、`foot` = 壁裾までの床（halfWidth − `CUT_WALL_FOOT`。砂利の路肩が `CUT_FOOT_RING_MIN` 0.3 m 以上ある県道の
  切通しだけ = `cutHasFootRing`）、`road` = ± `CUT_ROAD_HALF` 3.5（裾リングがある所はその `ROAD_SLACK` 0.15 m 内側、無ければ裾
  そのもの）。内側の 2 リングは壁になる端（始端キャップと、`daylight` 以外で終わる終端）の 1 m 内側から始まり、キャップにも
  側面にも**壁裾のカラム**を立てる（無いとキャップではフィル列 5 m、側面では路肩の全幅にわたって床が天端へ弦を張った
  — I6 レビュー R2 / V1）。路面リングは裾リングの中でキャップを 1 サンプル余分に引き、2 つの入れ子が同一のキャップ辺を
  共有しないようにする。
...
- **ツール**（I6 レビュー R8 / R9 で、落ちない検算を落ちるものに差し替え）: `dem-profile.mjs --cuts`（廊下ごとに 1 m 毎の床／
  場（有・無）／DEM の表、始端・終端・理由・最大深さ、**廊下の内側** 1.35 万点で埋めない・床を割らない・壁裾 0.6 + 0.25 m
  より内はビット同一の床、**標本器の閉じ込め** 2 m 格子 219 万点で多角形 + 裾の外は `cutAt` null かつ `y === yNoCut`）、
  `infield-smoke.mjs --cuts`（20 廊下: 終端 30〜120 m（`SHORT_CUTS`）／ピット 7 m、床 = **場自身の `yNoCut`**(portal) − depth と
  終端 = 床 + grade · 長さ、BARRIERS 線・STANDS 足跡（OSM リング **+ 手置きスタンドの弦帯**）と交わらない、行（corridor / foot /
  road）がプランのリング、**壁裾の内側の床全面**（0.25 m × 0.5 m 刻み 2.0 万点）で場が床・描かれた面が廊下の asphalt / gravel で
  場から 0.5 m 以内、キャップの 0.3〜1.0 m 外に凹み無し、廊下内で場は下がるだけ、路面フレーム 20,328 点で `cutAt` null、
  MARSHAL_POSTS / ops / 施設の足跡が廊下の中に無く、**置かれた実体も結合メッシュの頂点も**廊下の中で旧地面より下に無い、
  擁壁メッシュを読み直して床から 0.5 m 以上・裾の下まで・壁の無い側面が無い、坑口の数は `CUTS` から、buildMs、
  廊下の外の素の地形の沈み（settle の滲み、報告のみ））。
...
- **計測**（I6 レビュー後、Node no assets、両ティア同値）: 20 廊下（loopSouth 44 m / cut643 38.5 m wayEnd / chicaneLeft 57.5 m /
  worksNE 107 m / **worksSW 22 m** / r200service 9.5 m / 逆バンク 32〜33 m / ピット 7 m）、最大深さ 6.35 m（cut643 の d 10、
  シケイン側の地面の方が高い）。`buildMs.cuts` ≈ 0.22 s、plan 4.4 → 5.4 s、meshes 10.7 → **15.4** s（高; 低 4.4 → 5.3 /
  10.8 → 14.3; +30 リング・+cutRows・キャップの駅）— I6-a の 12.3 / 11.0 s から 3 s 増えたのはレビューの壁裾リングと
  リングの先端の駅（R2 / R7）の分で、地面の描画が正しくなった対価です（P7 のラスター細分と一緒に見直す。`pnpm perf` は
  別途ユーザーの GPU で）。三角形 65.1 → 69.5 万（地面）; scene-cost 高 4,407,762（予算 4,637,500）／低 2,521,658（2,594,000）— 予算内、再ベース無し。
  G1 mismatch 0（runtime 0）、**G2 0**、G5 231、G7 0、G11 0、G12 residual 0、`pnpm check` 両ティア green（許容 21、21 使用）。
  ALLOWANCES（両ティア `--suggest`、`WHY.cutWalls` — 壁裾 0.6 m は 10° より急で 45° の崖より緩い帯、かつ G4 は場の法線を
  丸めたメートル位置で読むので壁から 1 m 以内の床・路肩の三角形が壁の法線と比べられる）: I6-a が上げた値をレビューで
  測り直し、**下げたもの** G3 gravelArea 8.73 → 5.9（実測 5.61、I6-a は 8.31）・asphaltArea 4.4 → 4.26（4.05、4.19）、
  G4 gravelArea.steep 3,898 → 3,198（3,045、3,712）・asphaltArea.steep 3,128 → 2,859（2,722、2,979）・gravelBand.steep
  63 → 56（53、60）、**上げたもの** lane.steep 73 → 81（77、69: 場が廊下の全面で床になった分、構内道路ランプ脇の
  ピットレーンの三角形が壁の法線と比べられる）、据え置き grass.steep 2,796（2,699）・asphaltBand.steep 370（362）。
...
- **擁壁** `furniture-cut-walls`（cast、両側 = 廊下 × 2 = 40 本）: 廊下縁ポリライン（`CutField.corridor` = 地面行と同じ多角形）に
  ...
  （I6 レビュー R2 / V1 まではそうではありませんでした: 地面行に壁裾のリングが無く、描かれた床は路肩の全幅で天端へ弦を張って
  擁壁の面の前に 3〜4 m の土の斜面を出していた。文は場の話で、描かれた地面の話ではなかった）。
...
**I6 レビューの修正**（確定 18 件、採番 R1〜R9 / V1〜V9）:

- **場**: `CutField.locateFrame` がワールドフレームで、廊下の**内側**の点を取りこぼしていた（サンプル番号を始端→終端の弦で
  推定して ±2 サンプルしか見ないので、way が弦から 15〜35° 曲がる所では across に比例した along 誤差が窓を越える）。弦上の
  位置の二分探索 + `halfWidth / CUT_STEP + 2` サンプルの窓 + |a| 最小のサンプルを選ぶ、に（R1 / V1）。chicaneLeft の右壁脇で
  地面が床より 4.5 m 高かったのはこれ。トラックフレームでも同じ窓を使い、across は多角形の隅までの比で測る（駅の法線が数 m 先で
  交わる digitise の揺れで、裾の smoothstep が 0.3 m ずれていた）。
- **地面行**: 廊下に**壁裾のリング**（`{ cut, part: 'foot' }`、上の R6 の項）(R2 / V1)。
- **データ**: `worksSW` 4.0 m / 16 %（R4 / V3）、`STANDS A1_TEMP` の `sRange` を 130 → 110（`worksNE` のポータル (119, +28.5) の
  足跡から 4.5 m 空ける。R3 / V2）、`MARSHAL_POSTS` のシケイン待避路 (5140, +27) → **(5144, +18)**（廊下の床に立っていた。
  P タイヤバリア lat 13.1 と廊下の軸 10.9 m の間の芝。R5 / V4）、`PADDOCK_LAMPS.extra` (5520, −44) → (5545, −44)（gyakuTunnelR の
  ランプの床に立っていた。V6）。
- **置く側**: 廊下の多角形を `ctx.keepOutPolys` / `infieldFootprints` に（壁裾 + 1 m。森・散在・駐車車両・樹列。V7）、
  `banks.ts` は芝の場所・シート・テントを廊下から 1 m 空ける（`BankStats.inCut`、21 個。R6 / V5）、`paddock.ts` と
  `infield-ground.ts` のランプは `field.cutAt` が非 null の場所を捨てる（V6）。
- **プラン**: リングの先端の駅を、キャップが駅の法線に乗る廊下では先端の内側 `TIP_INSIDE` 0.02 m に置き、フィルの無い先端区間の
  駐車列は反転させない（旧: 反転して外側のリングの縁を越え、廊下リングの列を内側リングの先端に結ぶ幻の交差を作り、階段の頭の
  すぐ外に 1.1〜1.7 m のくさびの凹みを掘っていた。R7）。
- **メッシュ**（壁裾のリングが出した回帰）: ワールド部の三角形を**面積の大きい順**に出し、同じ部分の既に描いた三角形に
  重なる耳を落とす（候補の 3 辺の中点を重心へ 1 割戻した点で判定 — 辺を共有するだけの隣は当たらない）。廊下リングが
  自分の壁裾リングの外に残す 0.6 m の環の輪郭にはスパーがあり、earcut がそこで折り返した耳と、その耳が乗る三角形の
  両方が残って、砂利の面が 41 mm 差で二枚重なっていました（G2 4 点、s 5157 lat 46）。輪郭にスパーが無ければどの重心も
  兄弟の中に無いので、並べ替えも判定も結果を変えません。
- **ガード**: 上の「ツール」の通り、落ちない検算 3 つ（`floorStart` の同語反復、`hMin ≥ 0.5`、坑口の数）と中心線だけの標本を
  差し替え、`facilities-check` に **O13**（CUTS 行 vs スタンド・建物の足跡、marshal / masts / lamps / ops / 施設のアンカーが
  廊下の軸から halfWidth + 2 m、depth 2〜8・grade 0〜0.16・halfWidth 1〜6・window）を追加（R8 / R9 / V7）。
- **shot-presets**: I6 の 4 行はどれも名前のものを写していなかった（`works-tunnel` のカメラはピットビルの T1 鼻の中）。
  レビューが raycast で確かめた 11 行に差し替え（`works-portal-sw` / `cut643-heli` / `cut643-from-dunlop-side` /
  `loopSouth-portal` / `r200-portal` / `gyaku-ramp-paddock` / `gyaku-ramp-square` / `stair-pit-nippo` / `q2-gap` /
  `chicane-bridge` と `cut-chicane`）。`works-portal-ne` は A1_TEMP の新しい端を GPU で見てから（V9 / R12）。
- **積み残し**（V8、P7 へ）: `Terrain.settle` の `clampUnderSheets` は、地面の面が掛かる高さ格子の三角形の頂点 3 つとも下げるので、
  廊下の面が多角形の外 1 セル（高 13.3 m / 低 18.1 m）まで格子を引き下げます。そこに面が無い所（県道の路肩、構内道路ランプ脇の
  T1 インフィールド）では**素の地形が場より 3〜6 m 沈んで**見え、県道の日照端とその先の遠景リボンの間もその窪みです。覆うには
  廊下ごとに 2 セル（≈ 36 m）の縁取りの面が要りますが、loopSouth / chicaneLeft の日照端（lateral +56〜60）では 75 m の帯を
  越えて遠景の領分に入るので、P7 のラスター細分と一緒に決めます。`infield-smoke --cuts` が今は**報告だけ**します。
- **計画からの逸脱の記録**（R14）: 階段ピットが 1 リングでなく asphalt の 2 重リングであること・`{ cut }` リングがラスターの
  範囲を広げないことは上の項の通り、`worksSW` の長さと `worksNE` のポータルは上の「データ」、CUTS の facilities-check 規則は
  O13 で埋めた。**`CutDef.wall`（`'concrete' | 'slope'`）は誰も読まない死んだデータ**です（全行 concrete、法面の切通しは未実装）。
- **低優先で見送り**: R10（黒箱の 9 点標本と `interiorMat` の DoubleSide）、R11（1 m の cut 行が全周の駅を増やす分の計測 —
  上の 計測 に build 時間だけ記録）、R13 の `wall` の削除、V10（擁壁の tint を白へ — GPU で確認してから）、
